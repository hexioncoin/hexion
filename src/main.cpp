...
static const int64_t nTargetTimespan = 60 * 60;  // Hexion - every 1 hour
unsigned int nTargetSpacing = 1 * 60; // Hexion - 1 minute
static const int64_t nInterval = nTargetTimespan / nTargetSpacing;

static const int64_t nDiffChangeTarget = 1;

unsigned int nStakeMinAge = 1 * 60 * 60; // Hexion - 1 hours
unsigned int nStakeMaxAge = -1; // Hexion - unlimited
unsigned int nModifierInterval = 10 * 60; // Hexion - time to elapse before new modifier is computed
...
const string strMessageMagic = "Hexion Signed Message:\n";
...
{
    int64_t nSubsidy = 347.22222222222222222222222222222 * COIN;
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Hexion", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "13 - nov - 2014";
        CTransaction txNew;
        txNew.nTime = 1415875939;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1415875939;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = !fTestNet ? 420559 : 420559;
        ...
        assert(block.hashMerkleRoot == uint256("0x78d7510391206233b63c05112e430de46bf7de4a99026552ccddc5b90449fca4"));
